...
export default function LoadingSkeleton({
  ...
  if (variant === "hero") {
    return (
      <section className="relative min-h-screen overflow-hidden bg-[#08111c] pt-28 pb-16 lg:pt-36 lg:pb-20">
        <div className="mx-auto grid max-w-[1600px] grid-cols-1 items-center gap-12 px-4 sm:px-6 md:px-10 lg:grid-cols-2 lg:px-14 2xl:px-24">
          <div className="space-y-4">
            <div className="h-8 w-44 animate-pulse rounded-full bg-cyan-100/20" />
            <div className="h-16 w-full animate-pulse rounded-xl bg-white/15" />
            <div className="h-16 w-4/5 animate-pulse rounded-xl bg-white/15" />
            <div className="h-8 w-2/3 animate-pulse rounded-xl bg-white/10" />
            <div className="mt-6 flex gap-3">
              <div className="h-11 w-36 animate-pulse rounded-full bg-cyan-100/20" />
              <div className="h-11 w-36 animate-pulse rounded-full bg-white/10" />
            </div>
          </div>
          <div className="mx-auto h-[290px] w-[290px] animate-pulse rounded-full bg-white/10 sm:h-[360px] sm:w-[360px] xl:h-[430px] xl:w-[430px]" />
        </div>
      </section>
    );
  }
  ...
  if (variant === "about") {
    return (
      <div className="mx-auto max-w-[1600px] animate-pulse">
        <div className="mb-8 h-10 w-64 rounded-xl bg-slate-200" />
        <div className="grid gap-8 md:grid-cols-2">
          ...
          <div className="rounded-3xl bg-white p-8 shadow">
            <div className="grid grid-cols-2 gap-4">
              {[1, 2, 3, 4].map((item) => (
                <div key={item} className="h-16 rounded bg-slate-200" />
              ))}
            </div>
          </div>
        </div>
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="animate-pulse rounded-3xl border border-cyan-100 bg-white p-6 shadow-md">
