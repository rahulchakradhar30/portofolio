        ...
        {loading ? (
          ...
          <ExpandableSection collapsedMaxHeightPx={900}>
            {visibleSkills.length === 0 ? (
              <div className="text-center font-bold text-lg">{siteCopy.skillsEmpty}</div>
            ) : (
              <ScrollContainer
                config={content?.scrollConfigs?.skills}
                verticalClassName="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 md:gap-8"
                horizontalItemClassName="shrink-0 w-[260px] sm:w-[320px] snap-start"
