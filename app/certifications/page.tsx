  ...
  return (
    <main className="section-surface min-h-screen px-4 pb-20 pt-24 sm:px-6 sm:pt-28 lg:px-10">
      <div className="mx-auto max-w-[1600px]">
        ...
        <h1 className="mb-3 bg-gradient-to-r from-[#0d1b2d] to-[#0f766e] bg-clip-text text-4xl font-black text-transparent sm:text-5xl md:text-6xl">All Certifications</h1>
        <p className="mb-10 max-w-2xl text-sm leading-relaxed text-slate-600 sm:text-base">Credentials and achievements with full details and evidence.</p>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 sm:gap-8">
            {certifications.map((cert, index) => (
              <motion.article
                ...
                className="overflow-hidden rounded-2xl border border-cyan-100 bg-white shadow-md sm:rounded-3xl"
              >
                <div className="relative h-44 bg-gradient-to-br from-cyan-100 to-emerald-100 sm:h-52">
                  {cert.image ? <Image src={cert.image} alt={cert.title} fill className="object-cover" /> : null}
                </div>
                <div className="p-5 sm:p-5">
                  <h2 className="text-base font-bold text-slate-900 sm:text-lg">{cert.title}</h2>
