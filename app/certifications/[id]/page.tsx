...
export default function CertificationDetailPage() {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-[#7a5f47]">Loading certification...</div>;
  if (!certification) return <div className="min-h-screen flex items-center justify-center text-[#7a5f47]">Certification not found.</div>;
        ...
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="rounded-3xl border border-[#7a5f47]/12 bg-white p-5 shadow-2xl sm:p-6 md:p-8">
          ...
          {certification.image && (
            <button
              ...
              className="relative mt-6 h-56 w-full overflow-hidden rounded-2xl border border-[#7a5f47]/12 sm:h-72"
          ...
          {certification.description && <p className="mt-6 text-sm leading-relaxed text-[#6a5846] sm:text-base">{certification.description}</p>}

          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            {certification.credentialUrl && (
              <a href={certification.credentialUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full bg-[#8d6b4e] px-4 py-2 text-sm font-semibold text-[#fffaf3]">
                <ExternalLink className="h-4 w-4" />
                View Credential
              </a>
            )}
            {certification.linkedinUrl && (
              <a href={certification.linkedinUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full border border-[#7a5f47]/12 px-4 py-2 text-sm font-semibold text-[#5f4a38]">
                <ExternalLink className="h-4 w-4" />
                View LinkedIn
              </a>
            )}
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 sm:gap-4">
              {certification.galleryImages.map((image, idx) => (
                <button
                  ...
                  className="relative h-40 overflow-hidden rounded-2xl border border-[#7a5f47]/12 sm:h-48"
          ...
          <section className="mt-10 rounded-3xl border border-[#7a5f47]/12 bg-white p-5 sm:p-6 md:p-8">
            ...
            <div className="grid gap-4 md:grid-cols-2 sm:gap-6">
              {youtubeIds.map((id, idx) => (
                <div key={`${id}-${idx}`} className="aspect-video overflow-hidden rounded-2xl border border-[#7a5f47]/12">
                  <iframe
