...
export async function POST(request: NextRequest) {
  try {
    ...
    const limit = await asyncEnforceAdminRateLimit({ request, scope: 'admin-send-otp', max: 6, windowMs: 60_000 });
    ...
    // Use Nodemailer via sendMail instead of Resend
    try {
      await sendMail({
        to: email,
      ...
      console.error('Failed to send OTP via mail:', mailError);
      
      // Fallback: log OTP in development mode so you can still log in
      if (process.env.NODE_ENV === 'development') {
        console.log(`\n\n[DEV MODE] OTP for ${email}: ${otp}\n\n`);
      } else {
        throw mailError; // Re-throw in prod to show error
      }
    }
